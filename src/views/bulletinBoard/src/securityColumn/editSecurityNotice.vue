<template>
  <div>
    <el-row type="flex" :gutter="20" style="padding-left: 20px">
      <el-col :span="4">
        <h2 class="subtitle" style="margin-top: 15px; margin-left: 5px">
          <el-button type="text" @click="goback"><i class="el-icon-back subtitle"></i></el-button>修改安全专栏内容
        </h2>
      </el-col>
      <el-col :span="20" >
        <div style="padding: 20px 33px 0px 0px;float: right">
          <el-button class="el-green-button" @click="submitNewSecurityNotice"><i class="el-icon-search"></i>保存</el-button>
    ...
    <el-row type="flex" :gutter="20" justify="space-around" style="margin-top: 20px;padding-bottom: 20px">
      <el-col  :span="20">
        <el-form :model="newSecurityNotice"  label-width="150px">
          <el-form-item
            prop="title"
            label="标题:"
...
export default {
  ...
  created () {
    request({
      method: 'get',
      url: '/bulletin/showsafetycolumn',
      params: {
        id: this.newSecurityNotice.id
      }
    }).then(res => {
      if (res.data != null) {
        this.newSecurityNotice = res.data.data
    ...
    this.newSecurityNotice.content.replace(/\n/g, '<br>')
    console.log('编辑通知' +
  ...
  methods: {
    ...
    fileChange (file, fileList) {
      const existFile = fileList.slice(0, fileList.length - 1).find(f => f.name === file.name) || this.newSecurityNotice.files.slice(0, this.newSecurityNotice.files.length - 1).find(f => f.name === file.name)// 如果文件名重复
      if (existFile) {
    ...
    delFiles () {
      console.log(this.newSecurityNotice.files)
      for (let i = this.newSecurityNotice.files.length - 1; i >= 0; i--) {
        if (this.newSecurityNotice.files[i].checked === true) {
          this.delfiles.push(this.newSecurityNotice.files[i].name)
          this.newSecurityNotice.files.splice(i, 1)
        }
      }
      // 这个不能用数组下标
    ...
    * 3. 上传新的文件
    * */
    submitNewSecurityNotice () {
      // 更改内容
      const data = new FormData()
      data.append('id', this.newSecurityNotice.id)
      data.append('newtitle', this.newSecurityNotice.title)
      data.append('content', this.newSecurityNotice.content)
      data.append('newdate', this.newSecurityNotice.date)
      request({
        method: 'post',
        url: '/bulletin/updatesafetycolumn',
        data: data
      }).then(res => {
        console.log(res)
        if (res.data.message === 'SUCCESS') {
          ...
          if (this.delfiles.length !== 0) {
            ...
            fd0.append('id', this.newSecurityNotice.id)
            this.delfiles.forEach(item => {
              fd0.append('filename', item)
            })
            request({
              method: 'post',
              url: '/bulletin/deletesafetycolumnfiles',
              data: fd0
            }).then(res => {
              console.log(res)
              if (res.data.message === 'SUCCESS') {
          ...
          if (this.uploadFiles.length !== 0) {
            ...
            fd1.append('id', this.newSecurityNotice.id)
            this.uploadFiles.forEach(item => {
              fd1.append('files', item.raw)
            })
            request({
              method: 'post',
              url: '/bulletin/uploadsafetycolumnfiles',
              data: fd1
            }).then(res => {
              console.log(res)
              if (res.data.message === 'SUCCESS') {
    ...
    successUpload (response, file, fileList) {
      ...
      this.newSecurityNotice.files.push(file)
    },
    handlePreview (file) {
      console.log(file)
    },
