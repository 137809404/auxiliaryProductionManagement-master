<template>
  <div style="height: 95%; overflow-y: auto; overflow-x: hidden">
    <el-row type="flex" :gutter="20" style="padding-left: 20px">
      ...
      <el-col :span="20">
        <div style="padding: 20px 20px 15px 0px;float: right">
          <el-button class="el-blue-button" @click="downloadSecurityColumnDox"><i class="el-icon-document"></i>&nbsp;导出</el-button>
        </div>
      </el-col>
    </el-row>
    ...
    <el-row type="flex" justify="space-around">
      <el-col :span="6" >
        <h2> {{securityColumn.title}}</h2>
      </el-col>
    </el-row>
    <el-row type="flex" justify="end">
      <el-col :span="6" >
        <span style="color: #d3dce6">发布时间：{{securityColumn.date}}</span>
      </el-col>
    </el-row>
    <el-row style="height:65vh;overflow-y: auto">
      <el-row type="flex" justify="space-around" style="margin-top: 20px">
        <el-col :span="16" >
          <div class="pre-line" style="font-size: 20px" v-html="securityColumn.content"></div>
        </el-col>
      </el-row>
      <el-row type="flex" justify="space-around" style="margin-top: 30px">
        <el-col :span="16" v-if="havePic">
          <el-carousel :interval="4000" type="card" v-for="file in securityColumn.files" :key="file.name">
            <el-carousel-item @click="handlePictureCardPreview(file.url)">
              <el-image style="width: 100%; height: 100%" fit="fill" v-if="matchType(file.name)==='image'" :src="file.url"></el-image>
            </el-carousel-item>
          </el-carousel>
          <span v-for="file in securityColumn.files" :key="file.url" style="margin-right: 10px">
...
export default {
  name: 'showsecurityColumn',
  data () {
    return {
      securityColumnId: this.$route.query.data,
      dialogImageUrl: '',
      dialogVisible: false,
      securityColumn: {
        id: 1,
  ...
  methods: {
    ...
    downloadFiles () {
      this.securityColumn.files.forEach(file => {
    ...
    downloadSecurityColumnDox () {
      request({
        method: 'post',
        url: '/bulletin/printsafetycolumn',
        params: {
          id: this.securityColumnId
        }
      }).then(res => {
        // 用创建‘a’的方式下载附件
  ...
  created () {
    request({
      ...
      if (res.data != null) {
        this.securityColumn = res.data.data
        this.securityColumn.files.forEach((file) => {
        ...
        console.log(this.securityColumn)// 请求的返回体
      } else {
        console.log('未获取到数据')
      }
    }).catch((error) => {
    ...
    this.securityColumn.content.replace(/\n/g, '<br>')
    console.log('显示通知' +
